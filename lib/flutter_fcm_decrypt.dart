...
//Will get the current path of the package to run it on pure dart.
...
/// This function will get the path of this package
Future<String> getPackagePath() async {
  String path = '';
  var packageConfig = await findPackageConfig(Directory.current);
  if (packageConfig != null) {
    Package package = packageConfig.packages
        .firstWhere((element) => element.name == 'flutter_fcm_decrypt');
    path = package.packageUriRoot.path.toString().replaceAll('/lib/', '');
  }
  return path;
}
...
final DynamicLibrary _dylib = () {
  ...
  if (Platform.isMacOS || Platform.isIOS) {
    if (Platform.version.contains('arm64')) {
      ...
      return DynamicLibrary.open('$prefixPath$_libName' + '_64' + '.dylib');
    } else {
      return DynamicLibrary.open('$prefixPath$_libName.dylib');
    }
  }
  ...
  if (!isRunningOnFlutter()) {
    prefixPath = '$_currentPackagePath\\windows\\';
  }
  if (Platform.isWindows) {
    //TODO:: include '.\\windows\\' before lib name like  './lib/$_libName.dylib' for dart only run ..
    return DynamicLibrary.open('$prefixPath$_libName.dll');
  }
...
Future<SendPort> _helperIsolateSendPort = () async {
  ...
  await Isolate.spawn((SendPort sendPort) async {
    ...
    _currentPackagePath = await getPackagePath();
    final FlutterFcmDecryptBindings _bindings =
        FlutterFcmDecryptBindings(_dylib);
    final ReceivePort helperReceivePort = ReceivePort()
      ..listen((dynamic data) {
        // On the helper isolate listen to requests and respond to them.
        if (data is _SumRequest) {
          Pointer<Utf8> result =
              _bindings.decrypt_message(data.a.toNativeUtf8());
          final _SumResponse response =
              _SumResponse(data.id, result.cast<Utf8>().toDartString());
